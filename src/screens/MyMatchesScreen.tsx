...
const FILTER_CONFIG: Record<FilterType, { label: string; icon: keyof typeof Ionicons.glyphMap }> = {
  ...
  disputed: { label: 'Disputed', icon: 'alert-circle-outline' },
};
      ...
      <AnimatedCard index={index} style={styles.matchCard}>
        ...
        <View style={styles.detailsRow}>
          <View style={styles.detailItem}>
            <Ionicons name="location-outline" size={14} color={COLORS.TEXT_TERTIARY} />
            <Text style={styles.venueText} numberOfLines={1}>{item.venue_name}</Text>
          </View>
          <View style={styles.detailItem}>
            <Ionicons name="calendar-outline" size={14} color={COLORS.TEXT_TERTIARY} />
...
export function MyMatchesScreen({ navigation }: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {(Object.keys(FILTER_CONFIG) as FilterType[]).map((type, index) => (
          <FilterButton
            key={type}
            type={type}
